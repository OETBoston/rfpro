...
export class KnowledgeBaseStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: KnowledgeBaseStackProps) {
    ...
    const knowledgeBase = new bedrock.CfnKnowledgeBase(scope, 'KnowledgeBase', {
      ...
      name: `${process.env.CDK_STACK_NAME!}-kb`,
      ...
      // the properties below are optional
      description: `Bedrock Knowledge Base for ${process.env.CDK_STACK_NAME!}`,
    });

    // Ensure proper dependency order: collection -> custom resource (creates index) -> knowledge base
    ...
    const dataSource = new bedrock.CfnDataSource(scope, 'S3DataSource', {
      ...
      name: `${process.env.CDK_STACK_NAME!}-kb-datasource`,
